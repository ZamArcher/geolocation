...
class Storage:
    ...
    def get(self, key):
        ...
        # для простоты храню метрики в обычном словаре и сортирую значения
        # при каждом запросе, в реальном приложении нужно выбрать другую
        # структуру данных
        result = {}
...
class EchoServerClientProtocol(asyncio.Protocol):
    ...
    def process_data(self, data):
        ...
        # разбираю сообщения при помощи self.parser
        commands = self.parser.decode(data)

        # выполняю команды и запоминаю результаты выполнения
        responses = []
        for command in commands:
            resp = self.executor.run(*command)
            responses.append(resp)

        # преобразовываю команды в строку
